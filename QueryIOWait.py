...
cursor.execute("select AVG (round(iowait_total/1000000, 2)) as IOWAIT from DBA_HIST_SQLSTAT where sql_id = '"+ sql_id +"' and snap_id = (select max(snap_id) from DBA_HIST_SQLSTAT where sql_id = '"+ sql_id +"')")

mediaIO = cursor.fetchone()
mediaStr = str(mediaIO)
